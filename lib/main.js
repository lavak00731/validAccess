export class ValidAccess {
    ...
    //this function trigger another function
    ...
    //check a value against a fetch request
    async validaFetch(elem) {
    ...
    validateInput(elemToValidate, e) {
        const elem = elemToValidate ? elemToValidate : e.target;
        const validationStatus = elem.validity;
        this.backUpHelpText(elem);
                ...
                for (const dataKey in elemToValidate.dataset) {
                    switch (dataKey) {
                        ...
                        case 'validaTarget':
                            ...
                            break;
                        //fetch end point to check if data exists
                        case 'validaFetch': 
                            if(!e) {
                                this.debounce(this.validaFetch(elemToValidate), 500);                            
                            }                        
                            default:
                            break;
                    }
